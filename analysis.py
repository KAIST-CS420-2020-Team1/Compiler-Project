...
def desugar_lines(lines):
    return list(itertools.chian(*map(desugar_line, lines)))
...
# Derive list of blocks from a Body
# A block is simply represented by a list of declarations or statements
def body_to_blocks(body):
    # TODO How to take care of function calls
    # Likely no need to exclude function call itself from basic block
    lines = body.decls + body.stmts
    return list(map(lambda x: list(x[1]), itertools.groupby(lines, key=is_branching)))
